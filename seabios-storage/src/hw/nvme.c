...
nvme_get_next_sqe(struct nvme_sq *sq, u8 opc, void *metadata, void *data)
{
    if (((sq->head + 1) & sq->common.mask) == sq->tail) {
        dprintf(1, "submission queue is full");
        return NULL;
    }
...
nvme_create_io_sq(struct nvme_ctrl *ctrl, struct nvme_sq *sq, u16 q_idx, struct nvme_cq *cq)
{
    ...
    dprintf(1, "sq %p create dword10 %08x dword11 %08x\n", sq,
...
nvme_wait_csts_rdy(struct nvme_ctrl *ctrl, unsigned rdy)
{
    ...
    while (rdy != ((csts = ctrl->reg->csts) & NVME_CSTS_RDY)) {
        ...
        if (csts & NVME_CSTS_FATAL) {
            dprintf(1, "NVMe fatal error during controller shutdown\n");
            return -1;
        }
...
nvme_controller_enable(struct nvme_ctrl *ctrl)
{
    ...
    dprintf(1, "  admin submission queue: %p\n", ctrl->admin_sq.sqe);
    dprintf(1, "  admin completion queue: %p\n", ctrl->admin_cq.cqe);
    ...
    dprintf(1, "NVMe has %u namespace%s.\n",
    ...
    dprintf(1, "NVMe initialization complete!\n");
    return 0;
...
nvme_controller_setup(void *opaque)
{
    ...
    u32 version = reg->vs;
    dprintf(1, "Found NVMe controller with version %u.%u.%u.\n",
            version >> 16, (version >> 8) & 0xFF, version & 0xFF);
    dprintf(1, "  Capabilities %016llx\n", reg->cap);

    if (~reg->cap & NVME_CAP_CSS_NVME) {
        dprintf(1, "Controller doesn't speak NVMe command set. Skipping.\n");
        goto err;
    }
...
nvme_scan(void)
{
    ...
    foreachpci(pci) {
        ...
        if (pci->prog_if != 2 /* as of NVM 1.0e */) {
            dprintf(1, "Found incompatble NVMe: prog-if=%02x\n", pci->prog_if);
            continue;
        }
