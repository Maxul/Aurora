...
///
static void *pcnet32_pci = NULL;
void *pcnet32_ioaddr = NULL;
///
...
///
if (NULL == pcnet32_ioaddr && entry->dev == pcnet32_pci) {
pcnet32_ioaddr = (void *)addr;
}
///
...
///
    ...
    foreachpci(pci_tmp) {
        if (PCI_VENDOR_ID_AMD == pci_tmp->vendor && PCI_DEVICE_ID_AMD_LANCE == pci_tmp->device) {
            pcnet32_pci = pci_tmp;
        }
    }
///
...
///
if (pcnet32_ioaddr) {
    dprintf(1, "pcnet32 io address %p\n", pcnet32_ioaddr);
}
///
