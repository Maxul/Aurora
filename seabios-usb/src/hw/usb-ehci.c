                     ...
                     , struct usb_endpoint_descriptor *epdesc)
{
    ...
    dprintf(16, "ehci_alloc_intr_pipe %p %d\n", &cntl->usb, frameexp);
                  ...
                  , struct usb_endpoint_descriptor *epdesc)
{
    ...
    struct usb_ehci_s *cntl = container_of(
        usbdev->hub->cntl, struct usb_ehci_s, usb);
    dprintf(16, "ehci_alloc_async_pipe %p %d\n", &cntl->usb, eptype);
               ...
               , void *data, int datasize)
{
    ...
    dprintf(16, "ehci_send_pipe qh=%p dir=%d data=%p size=%d\n"
...
ehci_poll_intr(struct usb_pipe *p, void *data)
{
    ...
    SET_LOWFLAT(td->token, (ehci_explen(maxpacket) | QTD_STS_ACTIVE
                            | QTD_PID_IN | ehci_maxerr(3)));
dprintf(1, "ehci_poll_intr %p\n", GET_LOWFLAT(pipe->data) + maxpacket * pos);
    return 0;
}
