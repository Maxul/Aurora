...
struct eth_addr test_ethaddr =  {{6,2,9,3,8,1}};
struct eth_addr test_ethaddr2 = {{6,2,9,3,8,2}};
struct eth_addr test_ethaddr3 = {{6,2,9,3,8,3}};
struct eth_addr test_ethaddr4 = {{6,2,9,3,8,4}};
...
static void
sgx_arp_response(ip4_addr_t *adr)
{
  int k;
  ...
  SMEMCPY(&etharphdr->dipaddr, &sgx_ipaddr, sizeof(ip4_addr_t));
  ...
  ethernet_input(p, &sgx_netif);
}

void test_etharp_table()
{
  ...
  if (netif_default != &sgx_netif) {
    printf("This test needs a default netif\n");
    fail();
  }
