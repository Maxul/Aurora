...
SGX-FS \cite{DBLP:conf/cloudcom/BurihabwaFMS18} hardened a user-level filesystem in an enclave, which is more generic than \textsc{Aurora}. Similarly, Pesos~\cite{DBLP:conf/eurosys/KrahnTVKBF18} builds a trusted path between enclaves and a storage device and supports policy-based object protection. However, Pesos requires a programmable storage device named Kinetic Open Storage as the device end, while \textsc{Aurora} aims for general storage devices.
